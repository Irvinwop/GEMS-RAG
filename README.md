...
The GUI opens at `http://127.0.0.1:8765/` as one page with checkboxes for the three comparison RAGs and every configured RAG-capable text or vision model. Its question-source control defaults to the 150 immutable MUTCD-150 questions from the updated MRAG clone and also offers the 49 curated gold question/answer records from the supplied snapshot. MUTCD-150 is explicitly question-only: generated upstream answers are never treated as gold, grading ZIP rows carry `has_gold_answer=false`, and the authoritative MUTCD PDF is included for the final grader. The gold-reference oracle is disabled for that dataset. The comparison profile is locked to BM25, Microsoft GraphRAG, and PaperQA2 with injected context, `top_k=6`, and a 9,600-character context budget. The broader retriever and context-mode catalog remains available to CLI experiments. The RAG inference profile separately records the chat, embedding, and vision models used internally by model-assisted retrievers; it can target OpenAI or a keyless local OpenAI-compatible endpoint without changing the answer-model matrix or grader. Before answer generation, injected-context runs create a fingerprinted retrieval snapshot with one fsynced row per `(question, RAG)` pair. Every answer model then receives the exact same frozen evidence instead of querying the RAG again. Snapshot builds and answer runs both resume after interruption, repair a truncated final row, reject configuration or content mismatches, and block concurrent writers. Run setup includes the ignored output folder and ZIP filename; **Run / resume** creates the named GPT Pro ZIP when the answer matrix finishes. The browser restores the setup and progress after reload. The answer-model catalog covers current and prior OpenAI, Anthropic, xAI, and Qwen sizes, the manuscript's historical Qwen VLMs, and local OpenAI-compatible aliases. Media generators, speech-only or realtime models, embeddings, rerankers, and retired dated snapshots are intentionally excluded. API tokens are stored in the ignored `.env` file with mode `0600`; their values are never returned by the API. GraphRAG reuses `OPENAI_API_KEY` under the OpenAI RAG profile instead of requiring a separate service key. The credential-bearing server accepts loopback connections only.

Materialize selected answer models from the safe dry-run template, then run the locked resumable pipeline:

```bash
PYTHONPATH=src .venv/bin/python -m gems_rag.cli materialize \
  configs/mutcd150-comparison.json \
  --model openai:gpt-5-mini,max_tokens=1200 \
  --output data/working/mutcd150-comparison/materialized_config.json

PYTHONPATH=src .venv/bin/python -m gems_rag.cli retrieval-snapshot build \
  data/working/mutcd150-comparison/materialized_config.json

PYTHONPATH=src .venv/bin/python -m gems_rag.cli mutcd-comparison run \
  data/working/mutcd150-comparison/materialized_config.json
```
...
Local LightRAG and RAG-Anything commands generated by the harness enable the upstream structured JSON entity-extraction mode. This avoids delimiter degeneration on small local models, and the extraction mode is included in each index completion fingerprint.
