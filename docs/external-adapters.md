...
.venv/bin/python scripts/query_graphrag_index.py index --method fast
...
Local backend profiles cap each GraphRAG query completion at 2,048 tokens through the generated model `call_args`; direct adapter calls can override this with `--llm-max-tokens`. Initialization assigns community reports a separate deterministic completion profile capped at 768 generated tokens and requests 300-word reports by default, preventing thousands of oversized summaries without truncating later queries. It removes the upstream community-report few-shot blocks because small local models can copy their synthetic Enron or event examples into unrelated MUTCD reports, and asks for 2-4 distinct concise findings to avoid repetition loops caused by the upstream prompt's conflicting 5-10 multi-paragraph and 300-word requirements. Use `init --keep-community-prompt-examples` only when deliberately reproducing the exact upstream prompts. Each indexing workflow's cache partition includes its model profile, so changing a model or call setting preserves resumability without reusing stale completions. Settings and all indexing-prompt checksums are included in the completion marker, so changing a prompt invalidates stale readiness. Override the report bounds and sampling with `init --community-report-max-tokens N --community-report-max-length N --community-report-temperature N`.
