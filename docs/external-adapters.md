...
Shared-corpus indexing groups text, figures, and tables into deterministic 25-page documents. Each retry skips page batches whose text and multimodal stages are both complete, while `.gems_rag_raganything_attempt.json` binds partial artifacts to the exact corpus, partition size, document ID, model settings, and endpoint. A changed input or backend is rejected until `--force` starts a clean index; use `--batch-pages N` to deliberately select a different partition size. The maintained `patches/raganything-fail-closed.patch` also makes upstream batch and fallback processing raise when a modality item, LightRAG initialization, or completion-status write fails, so missing visual evidence cannot be published as a complete four-mode index.
