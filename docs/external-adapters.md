...
The bootstrap applies `patches/megarag-empty-graph.patch`, which makes the official merge and refinement stages accept pages with no extracted graph records instead of indexing an empty mapping or calling `asyncio.wait()` on an empty set. It also propagates extraction/refinement failures after upstream records the failed status, preventing a later success update from hiding the error. For full manuals, the patch lazily decodes page images and awaits each embedding batch instead of retaining every RGB page and eagerly queueing every GME call. The adapter defaults to one page and one active call per GME embedding batch, avoiding quadratic vision-attention allocations on unified-memory systems without changing the embedding model or scores. Already-batched page images use GME's direct `embed()` primitive, avoiding a fresh eight-worker Torch `DataLoader` for every page while retaining the same preprocessing and forward path. Page vectors are atomically checkpointed every eight embeddings and existing vector IDs are skipped on retry, bounding lost work after interruption. These fixes are required for cover, image-only, and full-corpus indexing.
...
Drift search performs substantially more dependent model calls than local, global, or basic search. Structured JSON queries default to 2 primer folds, 3 follow-ups, and 1 exploration depth so local-model ablations still exercise primer, dependent exploration, and reduction without launching the upstream maximum of roughly 65 calls. Use `query --drift-primer-folds 5 --drift-k-followups 20 --drift-depth 3` to reproduce Microsoft's defaults. The selected budget is included in JSON results, and the catalog pins the bounded values explicitly. Its catalog timeout remains 1,200 seconds so a slow local structured query and the upstream CLI fallback can finish without being killed at the ten-minute boundary.
...
The router sends `/v1/embeddings` to the embedding server, requests for `qwen2.5vl:3b` or image-bearing messages to the vision server, and ordinary completion/model-discovery requests to the chat server. It removes null-valued optional OpenAI embedding fields that strict local servers reject. Repeat `--vision-model MODEL` when the vision endpoint exposes additional model IDs.
...
Shared-corpus indexing groups text, figures, and tables into deterministic 25-page documents with distinct citation filenames. Each retry skips complete page batches and can resume an unfinished batch from a durably completed LightRAG text stage, while `.gems_rag_raganything_attempt.json` binds partial artifacts to the exact corpus, partition size, document ID, model settings, and endpoint. A changed input or backend is rejected until `--force` starts a clean index; use `--batch-pages N` to deliberately select a different partition size. The maintained `patches/raganything-fail-closed.patch` supplies the runtime role configuration required by current LightRAG and makes upstream batch and fallback processing raise when a modality item, LightRAG initialization, or completion-status write fails, so missing visual evidence cannot be published as a complete four-mode index.
...
The upstream package eagerly imports CUDA-only VLLM/GritLM and optional Bedrock/Transformers backends even when the configured clients are OpenAI-compatible. It also pins the unpublished `openai==1.91.1`, omits the Parquet engine required by its default vector store, recognizes API embedding models only when their IDs contain `text-embedding`, emits a null seed rejected by strict OpenAI-compatible servers, parses model JSON with regex plus `eval`, drops into an undeclared `ipdb` breakpoint when an embedding batch fails, and otherwise converts OpenIE failures into empty graph records. `patches/hipporag-lazy-optional-backends.patch` makes optional backends lazy, adds `pyarrow`, routes otherwise unrecognized embedding IDs such as `nomic-embed-text` through the configured OpenAI-compatible endpoint, supplies a deterministic seed when upstream leaves it unset, strictly decodes and normalizes fenced entity/triple JSON without executing it, replaces that debugger trap with a surfaced exception, fails the batch on NER or triple-extraction errors, uses a local endpoint's own tokenizer to enforce HippoRAG's configured embedding limit on oversized inputs, uses the published `openai==1.91.0`, and atomically publishes Parquet, OpenIE, and graph files. The retrieval-only Python 3.12 bootstrap therefore installs no unused CUDA stack, cannot expose a file interrupted mid-write, and does not guess token counts from characters when a local model provides exact tokenization.
