...
Local backend profiles cap each GraphRAG query completion at 2,048 tokens through the generated model `call_args`; direct adapter calls can override this with `--llm-max-tokens`. Initialization assigns community reports a separate completion profile capped at 512 generated tokens and requests 300-token reports by default, preventing thousands of oversized summaries without truncating later queries. It also removes the upstream community-report few-shot blocks because small local models can copy their synthetic Enron or event examples into unrelated MUTCD reports. Use `init --keep-community-prompt-examples` only when deliberately reproducing the exact upstream prompts. Settings and all indexing-prompt checksums are included in the completion marker, so changing a prompt invalidates stale readiness. Override the report bounds with `init --community-report-max-tokens N --community-report-max-length N`.
