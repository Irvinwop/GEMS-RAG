...
- `scripts/query_hipporag_index.py` runs the official HippoRAG 2 graph retriever with optional CUDA backends loaded lazily, explicit OpenAI-compatible service checks, and a corpus/model-bound completion sentinel so partial indexes remain resumable but never appear ready.
